...
impl Png {
    ...
    pub fn remove_first_chunk(&mut self, chunk_type: &str) -> anyhow::Result<Chunk> {
        let Some(index) = self
            .chunks
            .iter()
            .position(|c| c.chunk_type().bytes().eq(chunk_type.as_bytes()))
        else {
            anyhow::bail!("No chunk with matching type found")
        };
    ...
    pub fn chunk_by_type(&self, chunk_type: &str) -> Option<&Chunk> {
        self.chunks
            .iter()
            .find(|c| c.chunk_type().bytes().eq(chunk_type.as_bytes()))
    }

    pub fn as_bytes(&self) -> Vec<u8> {
        let mut result = Vec::new();

        result.extend(Self::STANDARD_HEADER);
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> anyhow::Result<Self> {
        ...
        if header.ne(&Self::STANDARD_HEADER) {
        ...
        while let Ok(()) = reader.read_exact(&mut buffer) {
            let chunk_size = (u32::from_be_bytes(buffer) + 8) as usize;
